...
object RichMethodRefl {
  ...
  ): List[RichMethodRefl] = apply(getMethodSymbol(symbol = RichRuntimeMirror().getClassSymbol(className), methodName = methodName))
  ...
  def apply (
    symbol: ru.Symbol,
    methodName: ru.TermName
  ): List[RichMethodRefl] = apply(getMethodSymbol(symbol = symbol, methodName = methodName))
  ...
  ): RichMethodRefl = new RichMethodRefl(methodSymbol)
  ...
  ): List[RichMethodRefl] = methodSymbols.map(apply)
...
class RichMethodRefl (
  ...
  def call[A] (
    ...
    require(methodSymbol.paramLists.isEmpty || argsToPass.size == methodSymbol.paramLists.head.size, s"too few/much parameter given (given=${argsToPass.size}, wanted=${methodSymbol.paramLists.head.size})")
    require(methodSymbol.owner.asClass.isCaseClass || methodSymbol.owner.asClass.isClass, "methods owner is neither a case class nor a class, so please provide an instance mirror")
  ...
  def matchesParameter (
    ...
    if (argsWithIdx.size != paramsWithIdx.size)
      return false

    def mapTypes(in: Seq[(ru.Type, Int)]) = (PrimitiveRefl.toScalaType(in.head._1), PrimitiveRefl.toScalaType(in(1)._1))
  ...
  private def reflectCtorMethodMirror[A] () (
    ...
    require(typeTag.tpe =:= ru.typeOf[Nothing] || methodSymbol.returnType <:< typeTag.tpe, s"passed generic type ($typeTag) is no assignable to ($methodSymbol)")
