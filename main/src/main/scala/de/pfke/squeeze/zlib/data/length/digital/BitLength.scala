...
class BitLength(
   ...
   *   - `x > 0` when  `this > that`
   */
  override def compare(that: BitLength) = data.compareTo(that.data)

  /**
   * Equals implementation.
   */
  override def equals(obj: scala.Any) = obj.isInstanceOf[BitLength] && compare(obj.asInstanceOf[BitLength]) == 0
