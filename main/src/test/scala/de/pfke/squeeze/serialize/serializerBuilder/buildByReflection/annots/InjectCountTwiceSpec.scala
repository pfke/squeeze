  ...
  "testing serializer for InjectCountTwiceMock type" when {
    checkThis[InjectCountTwiceMock](
      code = s"""
                ...
                |  ): de.pfke.squeeze.serialize.mocks.annots.InjectCountTwiceMock = {
                |    require(iter.len.toByte >= 4, s"[de.pfke.squeeze.serialize.mocks.annots.InjectCountTwiceMock] given input has only $${iter.len} bytes left, but we need 4 byte")
                |    // read iter
                ...
                |
                |  override def write(
                |    data: de.pfke.squeeze.serialize.mocks.annots.InjectCountTwiceMock,
                |    hints: SerializerHint*
                |  )(
                |    implicit
                |    byteOrder: ByteOrder,
                ...
                |    version: Option[PatchLevelVersion]
                |  ): Unit = {
                |    require(findOneHint[ByteStringBuilderHint](hints = hints).nonEmpty, s"[de.pfke.squeeze.serialize.mocks.annots.InjectCountTwiceMock] given input has no ByteStringBuilderHint")
