    ...
    checkThis[asBitfield_mock_unaligned_blocks_multi](
      code =
        s"""
           ...
           |    require(iter.len.toByte >= 22, s"[de.pfke.squeeze.serialize.mocks.asBitfieldSpecs.asBitfield_mock_unaligned_blocks_multi] given input has only $${iter.len} left, but we need 22 byte")
           |    // read iter
