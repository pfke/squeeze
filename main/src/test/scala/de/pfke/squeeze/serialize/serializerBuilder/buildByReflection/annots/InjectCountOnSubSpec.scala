    ...
    checkThis[InjectCountOnSubMock](
      code = s"""
                ...
                |class InjectCountOnSubMockSerializer
                 |  extends CompiledSerializer[de.pfke.squeeze.serialize.mocks.annots.InjectCountOnSubMock] {
                ...
                |    require(iter.len.toByte >= 6, s"[de.pfke.squeeze.serialize.mocks.annots.InjectCountOnSubMock] given input has only $${iter.len} left, but we need 6 byte")
                |    // read iter
