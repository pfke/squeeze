  ...
  "testing with simple Byte type [ByteOrder.BIG_ENDIAN]" when {
    ...
    "read+write bit(s)" should {
      ...
      "[write] should return a ByteString with correct length" in {
        writeBitString[Short](tto, 5, 0x13).length should be(4)
      }

      "[write] should return correct packed ByteString" in {
        writeBitString[Short](tto, 2, 0x13) should be(ByteString(0, 0, 0, 0x03))
      }
    }
    ...
    "read+write byte(s)" should {
      ...
      "[write] should return a ByteString with correct length" in {
        writeByteString[Short](tto, 0x1321).length should be(2)
      }

      "[write] should return correct packed ByteString" in {
        writeByteString[Short](tto, 0x5413) should be(ByteString(0x54, 0x13))
      }
    }
  }
}
