...
import de.pfke.squeeze.serialize.mocks.annots.{InjectCountOnSubMock, SubInjectCountOnSubMock}
import de.pfke.squeeze.serialize.serializerCompiler.BaseCompilerSpec

class InjectCountOnSubSpec
  extends BaseCompilerSpec {
  "testing with simple InjectCountOnSubMock type [ByteOrder.BIG_ENDIAN]" when {
    implicit val byteOrder: ByteOrder = ByteOrder.BIG_ENDIAN
    implicit val version: None.type = None

    val tto = createTTO[InjectCountOnSubMock]()
    val value = InjectCountOnSubMock(
    ...
    "read+write bit(s)" should {
      ...
      "[write] should throw an exception" in {
        an[IllegalArgumentException] shouldBe thrownBy(writeBitString[InjectCountOnSubMock](tto, 5, value))
      }
    }
