  ...
  "testing serializer for InjectTotalLengthOnSubMock type" when {
    checkThis[InjectTotalLengthOnSubMock](
      code = s"""
                ...
                |  extends Serializer[de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock] {
                |  override def objectTypeInfo = GeneralRefl.generateTypeInfo[de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock]
                |
                |  override def read(
                ...
                |  ): de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock = {
                |    require(iter.len.toByte >= 10, s"[de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock] given input has only $${iter.len} bytes left, but we need 10 byte")
                |    // read iter
                |    val _1stParam = serializerContainer.read[de.pfke.squeeze.serialize.mocks.SubInjectTotalLengthOnSubMock](iter)
                ...
                |
                |  override def write(
                |    data: de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock,
                |    hints: SerializerHint*
                |  )(
                |    implicit
                |    byteOrder: ByteOrder,
                ...
                |    version: Option[PatchLevelVersion]
                |  ): Unit = {
                |    require(findOneHint[ByteStringBuilderHint](hints = hints).nonEmpty, s"[de.pfke.squeeze.serialize.mocks.InjectTotalLengthOnSubMock] given input has no ByteStringBuilderHint")
                |    serializerContainer.write[de.pfke.squeeze.serialize.mocks.SubInjectTotalLengthOnSubMock](data._1stParam, hints = hints:_*)
