  ...
  "simple regex match file instance (correct defined regex)" when {
    "reading" should {
      "find 1st line regex" in {
        new SimpleRegexMatchFile(createMock(), pattern = List("""^(produkte)=\"(.*)\"""".r)).read().get("produkte").get should be ("Fritz_Box_HW185")
      }
      "find regex in the middle" in {
        new SimpleRegexMatchFile(createMock(), pattern = List("""^(oem_liste)=\"(.*)\"""".r)).read().get("oem_liste").get should be ("avm 1und1")
      }
      "find last line regex" in {
        new SimpleRegexMatchFile(createMock(), pattern = List("""^(firmware_xterm_position)=(.*)""".r)).read().get("firmware_xterm_position").get should be ("0+0")
      }
