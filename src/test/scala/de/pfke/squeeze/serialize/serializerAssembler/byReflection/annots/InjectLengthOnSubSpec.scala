...
import de.pfke.squeeze.testing.mocks.InjectLengthOnSubMock
    ...
    "testing serializer for InjectLengthOnSubMock type" when {
      ...
      checkThis[InjectLengthOnSubMock](
        code = s"""
                  ...
                  |  override def objectTypeInfo = GenericOps.getTypeInfo[de.pintono.tools.squeeze.core.mocks.InjectLengthOnSubMock]
                  |
                  |  override def read(
